...
RUN="java -jar $DIR/target/busrouteservice.jar"
...
stop() {
  if [ ! -f "$PIDFILE" ] || ! kill -0 $(cat "$PIDFILE" | sed -n -e 's#.*PID \([0-9]*\) .*#\1#p'); then
    echo 'Service not running' >&2
    return 1
  fi
  kill -15 $(cat "$PIDFILE" | sed -n -e 's#.*PID \([0-9]*\) .*#\1#p') && rm -f "$PIDFILE"
}
